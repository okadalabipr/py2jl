import os
import re
from py2jl import jl_souece
from py2jl import triming_tools

def convert_model(jl_dir,py_dir):
    #space_num = int(input('How many spaces do you use for an indent? (input integer):'))
...
def make_name2idx(jl_dir):
    ...
    name2idx = jl_souece.make_name2idx_header()
...
def make_parameters(jl_dir,py_dir,space_num):
    ...
    parameters = jl_souece.parameters_header()
    ...
    parameters = parameters + jl_souece.parameters_footer()
...
def make_variables(jl_dir,py_dir,space_num):
    ...
    variables = jl_souece.variables_header()
    ...
    variables = variables + jl_souece.variables_footer()
...
def make_param_const(jl_dir,py_dir,space_num):
    ...
    param_const = jl_souece.param_const_header()
    ...
    param_const += jl_souece.param_const_footer()
...
def make_initial_condition(jl_dir,py_dir,space_num):
    ...
    initial_condition = jl_souece.initial_condition_header()
    ...
    initial_condition += jl_souece.initial_condition_footer()
...
def make_differential_equation(jl_dir,py_dir,space_num):
    ...
    with open(py_dir,mode='r') as f:
        lines = f.readlines()
    lines = triming_tools.lines_triming(lines,space_num)
    lines = triming_tools.insert_end(lines,triming_tools.search_end(lines,space_num))

    differential_equation = jl_souece.differential_equation_header1()
    for i,line in enumerate(lines):
        if line.replace(' ','').find('v=')!=-1:
            differential_equation += str(int(re.sub("\\D", "", line[line.find('*'):]))-1)
            lines.pop(i)
            break
    differential_equation += jl_souece.differential_equation_header2()
    ...
    differential_equation += jl_souece.differential_equation_footer()
